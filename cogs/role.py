...
class Role(commands.Cog):
    ...
    async def addrole(self, ctx: commands.Context, member: discord.Member, role: discord.Role):
        ...
        embed = discord.Embed(
            title="<a:tick:1489157731393994854> Role Added",
    ...
    async def removerole(self, ctx: commands.Context, member: discord.Member, role: discord.Role):
        ...
        embed = discord.Embed(
            title="<a:tick:1489157731393994854> Role Removed",
    ...
    # Create a new role
    @commands.hybrid_command(name="createrole", description="Create a new role with a name and optional color.")
    @commands.has_permissions(manage_roles=True)
    @app_commands.guild_only()
    async def createrole(self, ctx: commands.Context, role_name: str, color: str = "blue"):
        ...
        embed = discord.Embed(
            title="<a:tick:1489157731393994854> Role Created",
    ...
    # List all roles
    @commands.hybrid_command(name="listroles", description="List all roles in the server.")
    @app_commands.guild_only()
    ...
    # Role info command
    @commands.hybrid_command(name="roleinfo", description="Get details about a specific role.")
    @app_commands.guild_only()
    async def roleinfo(self, ctx: commands.Context, role: discord.Role):
        embed = discord.Embed(title=f"🛠️ Role Info: {role.name}", color=role.color)
        embed.add_field(name="Role Name", value=role.name, inline=True)
        embed.add_field(name="Role ID", value=role.id, inline=True)
        embed.add_field(name="Created At", value=role.created_at.strftime("%b %d, %Y"), inline=True)
        embed.add_field(name="Color", value=str(role.color), inline=True)
        embed.add_field(name="Hoisted", value="<a:tick:1489157731393994854>" if role.hoist else "<a:Cross_:1489174755537064046>", inline=True)
        embed.add_field(name="Position", value=role.position, inline=True)
        embed.add_field(name="Mentionable", value="<a:tick:1489157731393994854>" if role.mentionable else "<a:Cross_:1489174755537064046>", inline=True)
        embed.add_field(name="Members", value=len(role.members), inline=True)
        await ctx.send(embed=embed)

    # Give role to all members
    @commands.hybrid_command(name="giveroleall", description="Assign a role to all members.")
    @commands.has_permissions(manage_roles=True)
    @app_commands.guild_only()
    async def giveroleall(self, ctx: commands.Context, role: discord.Role):
        ...
        if not members_without_role:
            return await ctx.send(f"<a:tick:1489157731393994854> All members already have the **{role.name}** role.")

        embed = discord.Embed(
            title="<a:tick:1489157731393994854> Role Assignment in Progress",
        ...
        for member in members_without_role:
            try:
                ...
                await ctx.send(f"<a:Cross_:1489174755537064046> Could not add role to **{member.name}** (Missing Permissions).")

        embed = discord.Embed(
            title="<a:tick:1489157731393994854> Role Assigned to All",
